...
impl_err!(
    TooManyRoles,
    "Can't add more roles to joinable; you have too many!",
    true
);
...
impl<'pool> JoinableRoles<'pool> {
    ...
    pub async fn joinable_roles(&self) -> crate::error::Result<Vec<RoleId>> {
        ...
        out.extend(s.into_iter().map(|r| RoleId::from(r as u64)));
        Ok(out)
    }
}
...
impl Module for RoleModule {
    fn info(&self) -> &ModInfo {
        ...
        static INFO: Lazy<ModInfo> = Lazy::new(|| {
            ModInfo::with_name("role", "allows users to self-manage roles.")
    ...
    async fn process(
        ...
        match &role_opts {
            RoleOpt::Join { .. } | RoleOpt::Leave { .. } => {
                ...
                let full_role = vrole.into_inner().to_role_cached(ctx).await.ok_or(RoleNotInCache)?;
                ...
                let mut mem = guild.member(ctx, orig.author.id).await?;
...
impl ModRoleOpt {
    ...
    pub fn extract_user(&self) -> Option<&str> {
        match self {
            ModRoleOpt::AddJoinable { .. } | ModRoleOpt::DelJoinable { .. } => None,
    ...
    pub fn is_assign(&self) -> bool {
        matches!(self, ModRoleOpt::Assign { .. })
    }
}
...
impl Module for ModRoleModule {
    fn info(&self) -> &ModInfo {
        ...
        static INFO: Lazy<ModInfo> = Lazy::new(|| {
            ModInfo::with_name(
                "mod-role",
                "allows moderators to assign/unassign roles, and to make/unmake roles assignable.",
            )
            .with_command(true)
    ...
    async fn process(
        ...
        let gid = orig.guild_id.unwrap();
        let role = VerifiedRole::from_str_with_ctx(opts.extract_role(), ctx, gid).await?;

        let full_role = role.into_inner().to_role_cached(ctx).await.ok_or(RoleNotInCache)?;
        ...
        match opts {
            ...
            _ => {
                ...
                if opts.is_assign() {
                    member.add_role(ctx, role.into_inner()).await?;
                    "Added role to user."
                } else {
                    member.remove_role(ctx, role.into_inner()).await?;
