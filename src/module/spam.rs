...
use crate::dispatch::config::{VerifiedRole, ValueType, FromStrWithCtx, VerifiedChannel, VerifiedUser};
use serenity::model::guild::Member;
use noisy_float::types::R64;
use std::{time, fmt};
use noisy_float::prelude::Float;
use crate::module::{Module, UnimplementedModule, ModInfo, Sensitivity, CHECKMARK_IN_GREEN_BOX};
...
impl PressureOp {
    pub fn user(&self) -> &str {
        match self {
            PressureOp::GetFor { user } => {
                user.as_ref()
            }
            PressureOp::SetFor { user, .. } => {
                user.as_ref()
            }
            PressureOp::ClearFor { user } => {
                user.as_ref()
            }
        }
    }
        ...
        match opts {
            SpamOpts::Clean { num, channel, who } => {
                ...
                let who = futures::stream::iter(who.as_ref())
                    ...
                    .await
                    .transpose()?
                    .or_else(|| orig.referenced_message.as_ref().map(|m| VerifiedUser::from_known(m.author.id)));

                let num_cleaned = clean_messages(dis, ctx,
                               num,
                               before,
                               gid,
                               channel,
            ...
            SpamOpts::Pressure { op } => {
                let user = VerifiedUser::from_str_with_ctx(
                    op.user(),
                    ctx,
                    gid)
                    .await?;
...
pub async fn clean_messages(dis: &Dispatch,
                            ctx: &Context,
                            how_many: ConstrainedU64<1, 100>, before: chrono::DateTime<Utc>,
                            in_guild: GuildId, in_channel: VerifiedChannel, for_user: Option<VerifiedUser>) -> crate::error::Result<usize> {

    let cv = dis.message_cache()
        .get(&in_guild)
        .ok_or(GuildNotInCache)?;

    let snapshot = cv.snapshot();

    let relevant_messages = snapshot.iter().rev()
        .cloned()
    ...
    let mids = full_ids
        .take(how_many.to_usize().unwrap()).collect::<Vec<_>>();

    let v = match &mids[..] {
        [] => {0},
        [id] => {
            in_channel.into_inner().delete_message(ctx, id.msg)
                .await
                .map(|_| 1)?
        }
        [vals @ ..] => {
            in_channel.into_inner().delete_messages(ctx, vals.iter().map(|m| m.msg))
                .await
                .map(|_| mids.len())?
        }
    };
